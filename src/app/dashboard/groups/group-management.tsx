...
function GroupEditor({ group }: { group: Group }) {
  ...
  return (
    <form action={action} className="rounded-2xl border border-[#d9e1f2] bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_2fr_auto_auto] md:items-end">
        ...
        <label className="flex items-center gap-2 pb-2.5 text-sm text-slate-700"><input name="active" type="checkbox" defaultChecked={group.active} className="h-4 w-4 accent-[#1b3272]" />Attivo</label>
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-[#d9e1f2] bg-white p-5 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-[#1b3272]">Nuovo gruppo</h2><CreateGroupForm />
      </section>
