-- Add a copy mode to position export.
-- Transfer remains the default behavior; copy creates the new contact without clearing source data.

drop function if exists public.export_contact_position(bigint, text[], bigint[]);
drop function if exists public.export_contact_position(bigint, text[], bigint[], text);
...
begin
  ...
  normalized_mode := coalesce(nullif(trim(p_mode), ''), 'transfer');
  if normalized_mode not in ('transfer', 'copy') then
    raise exception 'POSITION_EXPORT_MODE_NOT_ALLOWED:%', normalized_mode;
  end if;
  should_transfer := normalized_mode = 'transfer';
  ...
  if should_transfer
    and nullif(trim(source_contact.first_name), '') is null
...
revoke all on function public.export_contact_position(bigint, text[], bigint[], text) from public;
grant execute on function public.export_contact_position(bigint, text[], bigint[], text) to authenticated;

comment on function public.export_contact_position(bigint, text[], bigint[], text) is
  'Creates a new contact with selected position data. In transfer mode it clears fields and selected groups from the source; in copy mode it leaves source data untouched. References and event history remain on the source.';
